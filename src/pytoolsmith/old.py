...
# Need noinspect for _Literal/_Union
...
from sp_py_lib.utils.serialize import jencode
...
class ToolDefinition(BaseModel):
    ...
    injected_parameters: list[Literal["t_id", "user_id", "request_context"]] = []
...
def _get_type_for_parameter(
    ...
    for param_type in param_type_options:
        list_args = _get_list_options(param_type)
        param_type = _strip_aliases(param_type)

        param_json_type = _get_json_type(param_type)
        ...
        if param_json_type == "array" and list_args:
            ...
            item_schema, _ = _get_type_for_parameter(
                param_name="item",
    ...
    if not is_array_item:
        ...
        default_value = _create_default_value(param_info.default)
...
class ToolLibrary(BaseModel):
    """Defines a library of tools to be used for an LLM conversation."""

    tools: list[ToolDefinition]

    _tool_library: dict[str, ToolDefinition] = PrivateAttr(default=None)
    _tool_parameters: list[AwsBedrockToolSpecListObject] = PrivateAttr(default=None)

    def get_tool_from_name(self, name: str) -> ToolDefinition:
        return self._get_tool_library()[name]

    @property
    def tool_parameter_list(self) -> list[AwsBedrockToolSpecListObject]:
        if self._tool_parameters is None:
            self._tool_parameters = [
                AwsBedrockToolSpecListObject(toolSpec=t.build_json_schema())
                for t in self.tools
            ]
        return self._tool_parameters

    def _get_tool_library(self) -> dict[str, ToolDefinition]:
        if self._tool_library is None:
            self._tool_library = {t.name: t for t in self.tools}
...
def _extract_function_descriptions(docstring: str) -> str:
